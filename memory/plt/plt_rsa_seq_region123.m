function plt_rsa_seq_region123(rsa_seq,subject,plot_window,method)
...
% 
% Inputs:
%   rsa_seq: RSA results for all subjects
...
region1 = squeeze(mean(rsa_seq(:,1:200,:),2));
region2 = squeeze(mean(rsa_seq(:,451:500,:),2));
region3 = squeeze(mean(rsa_seq(:,501:700,:),2));

ylims = size(rsa_seq,1);
...
subplot(122)
region1 = rsa_seq(:,1:200,:);
region3 = rsa_seq(:,501:700,:);
